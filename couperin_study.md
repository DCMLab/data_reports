...
def summarize_groups_movements(df, column="preceding_movement_precise"):
    ...
    N = len(proportions)
    normalized_entropy = entropy / np.log2(N) if N > 1 else 0.0
    main_movements = [ix for ix in ("ascending", "descending", "leap", "same") if ix in proportions.index.values]
...
def summarize_degree_wise_movement(BN, column="preceding_movement_precise"):
    ...
    column_order = ["movement_entropy", "leap", "ascending", "descending", "same", "other_step"]
    result = result.pivot(columns="preceding_movement_precise", values="proportion")[column_order]
    return result
